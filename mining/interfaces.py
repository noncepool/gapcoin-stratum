...
class WorkerManagerInterface(object):
    ...
    def register_work(self, extranonce1_bin, merkle_root, difficulty, job_id, basediff):
        now = Interfaces.timestamper.time()
        work_id = WorkIdGenerator.get_new_id()
        self.job_log.setdefault(extranonce1_bin, {})[merkle_root] = (job_id, difficulty, basediff, now)
        return work_id
...
class ShareManagerInterface(object):
    ...
    def on_submit_share(self, worker_name, basediff, block_hash, difficulty, timestamp, is_valid, ip, invalid_reason, share_diff, job_id):
        log.info("%s [%s] diff(%f/%f) job_id(%s) share(%i) %s %s" % (worker_name, ip, share_diff, basediff, job_id, difficulty, 'valid' if is_valid else 'INVALID', invalid_reason))

        dbi.queue_share([worker_name, block_hash, difficulty, timestamp, is_valid, ip, invalid_reason, share_diff ])
 
    def on_submit_block(self, is_accepted, worker_name, basediff, block_hash, timestamp, ip, share_diff):
